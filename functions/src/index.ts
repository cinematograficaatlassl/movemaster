...
export const bootstrapWorkspace = onCall(async (req): Promise<BootstrapResult> => {
  try {
    ...
    const displayName = (req.auth.token.name as string | undefined) ?? "";
    ...
    if (!existing.empty) {
      ...
      return { workspaceId: doc.id, workspaceName: (doc.data().name as string) ?? "Workspace" };
    }
  ...
  const byTitleToRef: Record<string, FirebaseFirestore.DocumentReference> = {};
  ...
  for (const t of seedTasks) {
    ...
    const deps =
      t.dependenciesTitles?.map((title) => {
  ...
  batch.set(wsRef.collection("templates").doc(), {
    ...
    if (msg.includes("Cloud Firestore") || msg.includes("firestore") || msg.includes("Firestore")) {
      throw new HttpsError(
        "failed-precondition",
        "FIRESTORE_NOT_READY",
        "Activa Firestore en Firebase Console (Build → Firestore Database → Create database) y reintenta."
      );
    }
    throw new HttpsError("internal", "BOOTSTRAP_FAILED", msg || "Internal error");
  }
});
